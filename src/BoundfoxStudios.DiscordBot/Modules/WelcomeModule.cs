  ...
  public class WelcomeModule : EnableableModule
  {
    ...
    private async Task WelcomeUser(SocketGuildUser user)
    {
      ...
      await channel.SendMessageAsync(
        $"Hi {MentionUtils.MentionUser(user.Id)}! Welcome to Boundfox Studios. Please read the {MentionUtils.MentionChannel(rulesChannelId)}! And don't forget to set your own roles in {MentionUtils.MentionChannel(rolesChannelId)}. :-)");
      ...
      var message =
        ...
        $"{MentionUtils.MentionChannel(Options.CurrentValue.Modules.Welcome.RulesChannelId)}: Hier sind die Regeln. Bitte lese sie aufmerksam durch!\n" +
      ...
      message =
        ...
        $"{MentionUtils.MentionChannel(Options.CurrentValue.Modules.Welcome.RulesChannelId)}: Here are the rules. Please read them carefully.\n" +
